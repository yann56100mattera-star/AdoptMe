-- ═══════════════════════════════════════════════════════════════
-- 🐾 HIROSHI 738 — ADOPT ME ULTIMATE PREMIUM v4.0
-- By Hiroshi738 · discord.gg/qTGtM4Uyc
-- 200+ Features · 15 Tabs · Premium Design
-- ═══════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════
-- KEY SYSTEM WITH ANIMATED PIXEL BACKGROUND
-- ═══════════════════════════════════════════════════════════════
...
for i=1,50 do
    local px=Instance.new("Frame",GR)local sz=math.random(6,18)
    px.Size=UDim2.new(0,sz,0,sz)px.Position=UDim2.new(math.random(0,100)/100,0,math.random(0,100)/100,0)
    px.BackgroundTransparency=math.random(75,95)/100 px.BorderSizePixel=0
    local cols={Color3.fromRGB(0,200,100),Color3.fromRGB(0,255,150),Color3.fromRGB(100,255,200),Color3.fromRGB(0,180,255),Color3.fromRGB(255,220,0),Color3.fromRGB(255,100,200),Color3.fromRGB(150,100,255)}
    px.BackgroundColor3=cols[math.random(1,#cols)]
    Instance.new("UICorner",px).CornerRadius=UDim.new(0,2)
    task.spawn(function()while not OK and px.Parent do TW:Create(px,TweenInfo.new(math.random(30,60)/10,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{Position=UDim2.new(math.random(0,100)/100,0,math.random(0,100)/100,0),BackgroundTransparency=math.random(75,95)/100,Size=UDim2.new(0,math.random(5,20),0,math.random(5,20))}):Play()task.wait(math.random(30,60)/10)end end)
end
...
for i=1,15 do
    ...
    p.BackgroundColor3=Color3.fromRGB(0,255,150)p.BackgroundTransparency=math.random(20,45)/100 p.BorderSizePixel=0 p.ZIndex=3
    ...
    task.spawn(function()while not OK and p.Parent do TW:Create(p,TweenInfo.new(math.random(4,7),Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{Position=UDim2.new(math.random(5,95)/100,0,math.random(5,95)/100,0)}):Play()task.wait(math.random(4,7))end end)
end

-- Double scan lines
for i=1,2 do local sl=Instance.new("Frame",GR)sl.Size=UDim2.new(1,0,0,1+i)sl.BackgroundColor3=i==1 and Color3.fromRGB(0,255,150)or Color3.fromRGB(0,180,255)sl.BackgroundTransparency=0.6 sl.BorderSizePixel=0 sl.ZIndex=4
...
-- Key Card
local CD=Instance.new("Frame",BG)CD.Size=UDim2.new(0,380,0,330)CD.Position=UDim2.new(0.5,-190,0.5,-165)CD.BackgroundColor3=Color3.fromRGB(6,18,12)CD.BorderSizePixel=0 CD.ZIndex=10 Instance.new("UICorner",CD).CornerRadius=UDim.new(0,18)
local cds=Instance.new("UIStroke",CD)cds.Color=Color3.fromRGB(0,200,100)cds.Thickness=1.5

-- Rainbow accent bars top+bottom
for _,yy in ipairs({0,327}) do
    local bar=Instance.new("Frame",CD)bar.Size=UDim2.new(1,-40,0,3)bar.Position=UDim2.new(0,20,0,yy)bar.BackgroundColor3=Color3.fromRGB(0,200,100)bar.BorderSizePixel=0 bar.ZIndex=11 Instance.new("UICorner",bar).CornerRadius=UDim.new(1,0)
    local g=Instance.new("UIGradient",bar)g.Color=ColorSequence.new{ColorSequenceKeypoint.new(0,Color3.fromRGB(0,200,100)),ColorSequenceKeypoint.new(0.25,Color3.fromRGB(0,255,200)),ColorSequenceKeypoint.new(0.5,Color3.fromRGB(0,180,255)),ColorSequenceKeypoint.new(0.75,Color3.fromRGB(255,220,0)),ColorSequenceKeypoint.new(1,Color3.fromRGB(255,100,200))}
    task.spawn(function()local r=0 while not OK and bar.Parent do r=(r+2)%360 g.Rotation=r task.wait(0.03)end end)
end

-- Logo with rainbow pulse
local lg=Instance.new("Frame",CD)lg.Size=UDim2.new(0,60,0,60)lg.Position=UDim2.new(0.5,-30,0,20)lg.BackgroundColor3=Color3.fromRGB(0,200,100)lg.BorderSizePixel=0 lg.ZIndex=12 Instance.new("UICorner",lg).CornerRadius=UDim.new(0,16)
...
local glowR=Instance.new("Frame",CD)glowR.Size=UDim2.new(0,80,0,80)glowR.Position=UDim2.new(0.5,-40,0,10)glowR.BackgroundColor3=Color3.fromRGB(0,200,100)glowR.BackgroundTransparency=0.85 glowR.BorderSizePixel=0 glowR.ZIndex=11 Instance.new("UICorner",glowR).CornerRadius=UDim.new(0,20)
task.spawn(function()local h=0 while not OK and glowR.Parent do h=(h+0.01)%1 TW:Create(glowR,TweenInfo.new(0.5),{BackgroundColor3=Color3.fromHSV(h,0.7,1)}):Play()TW:Create(glowR,TweenInfo.new(1.2,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{BackgroundTransparency=0.7,Size=UDim2.new(0,88,0,88),Position=UDim2.new(0.5,-44,0,6)}):Play()task.wait(1.2)TW:Create(glowR,TweenInfo.new(1.2,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{BackgroundTransparency=0.9,Size=UDim2.new(0,80,0,80),Position=UDim2.new(0.5,-40,0,10)}):Play()task.wait(1.2)end end)
...
kL("v4.0 · 200+ FEATURES",146,Enum.Font.GothamMedium,7,Color3.fromRGB(100,255,180))

local kib=Instance.new("Frame",CD)kib.Size=UDim2.new(0,270,0,40)kib.Position=UDim2.new(0.5,-135,0,170)kib.BackgroundColor3=Color3.fromRGB(10,25,16)kib.BorderSizePixel=0 kib.ZIndex=12 Instance.new("UICorner",kib).CornerRadius=UDim.new(0,10)
local kis=Instance.new("UIStroke",kib)kis.Color=Color3.fromRGB(35,70,50)
local klk=Instance.new("TextLabel",kib)klk.Size=UDim2.new(0,32,1,0)klk.BackgroundTransparency=1 klk.Text="🔒" klk.Font=Enum.Font.Gotham klk.TextSize=15 klk.TextColor3=Color3.fromRGB(0,200,100)klk.ZIndex=13
local ki=Instance.new("TextBox",kib)ki.Size=UDim2.new(1,-40,1,0)ki.Position=UDim2.new(0,34,0,0)ki.BackgroundTransparency=1 ki.Text="" ki.PlaceholderText="Enter your key..." ki.PlaceholderColor3=Color3.fromRGB(70,130,90)ki.Font=Enum.Font.GothamMedium ki.TextSize=13 ki.TextColor3=Color3.new(1,1,1)ki.TextXAlignment=Enum.TextXAlignment.Left ki.ClearTextOnFocus=false ki.ZIndex=13
...
local kb=Instance.new("TextButton",CD)kb.Size=UDim2.new(0,270,0,38)kb.Position=UDim2.new(0.5,-135,0,220)kb.BackgroundColor3=Color3.fromRGB(0,200,100)kb.BorderSizePixel=0 kb.Text="VERIFY KEY" kb.Font=Enum.Font.GothamBold kb.TextSize=13 kb.TextColor3=Color3.new(1,1,1)kb.AutoButtonColor=false kb.ZIndex=12 Instance.new("UICorner",kb).CornerRadius=UDim.new(0,10)
Instance.new("UIGradient",kb).Color=ColorSequence.new(Color3.fromRGB(0,160,80),Color3.fromRGB(0,230,140))
local ke=kL("",268,Enum.Font.GothamMedium,10,Color3.fromRGB(255,55,75))
local ka=kL("3 attempts remaining",286,Enum.Font.Gotham,9,Color3.fromRGB(70,130,90))
kL("Protected by AES-256 · By Hiroshi738",306,Enum.Font.Gotham,7,Color3.fromRGB(45,90,65))
...
local function DoV()if vf or OK or att<=0 then return end vf=true kb.Text="AUTHENTICATING..." task.wait(0.4)
    if ki.Text==KEY then OK=true ke.Text="" kb.Text="✓ ACCESS GRANTED" kb.BackgroundColor3=Color3.fromRGB(0,255,130)klk.Text="🔓" kSub.Text="Welcome, "..LP.Name kSub.TextColor3=Color3.fromRGB(0,255,130)task.wait(0.6)
        ...
        ke.Text="✗ Invalid" kb.Text="VERIFY KEY" kb.BackgroundColor3=Color3.fromRGB(0,200,100)ka.Text=att.." remaining"
        TW:Create(kis,TweenInfo.new(0.1),{Color=Color3.fromRGB(255,55,75)}):Play()
        for _,off in ipairs({-130,-140,-135})do TW:Create(kib,TweenInfo.new(0.04),{Position=UDim2.new(0.5,off,0,170)}):Play()task.wait(0.04)end
...
local CFG={Speed=false,SV=50,JP=false,JV=100,Fly=false,FS=60,Noclip=false,InfJump=false,AFK=true,GodMode=false,Tiny=false,Giant=false,Invis=false,AutoCollect=false,CR=50,CS=3,AutoCare=false,CareSpd=8,FB=false,NF=false,FvC=false,CF=70,FT=false,TV=14,Night=false,Sunset=false,NoShadow=false,ESPNm=false,ESPBx=false,ESPDst=false,ESPTr=false,ESPHp=false,RbESP=false,SP=nil}
...
local collectCount=0 local careCount=0 local farmCycles=0 local Session=tick()local Spec=nil local AutoFarm=false local FarmIdx=1

local PLACES={
    {n="🏠 Nursery",p=CFrame.new(-451,30,-68)},{n="🏥 Hospital",p=CFrame.new(-281,30,-1418)},
    {n="🍕 Pizza Shop",p=CFrame.new(-126,30,-1664)},{n="🏪 Pet Shop",p=CFrame.new(-196,30,-1505)},
    {n="🏫 School",p=CFrame.new(-305,30,-1504)},{n="🎁 Gifts",p=CFrame.new(-272,30,-1535)},
    {n="🌳 Park",p=CFrame.new(-447,30,-1729)},{n="⛺ Campsite",p=CFrame.new(-147,30,-1800)},
    {n="🏰 Castle",p=CFrame.new(188,52,-1650)},{n="🌊 Pool",p=CFrame.new(85,30,-1600)},
    {n="🎩 Hat Shop",p=CFrame.new(-200,30,-1600)},{n="🚢 Boat",p=CFrame.new(-735,30,-1400)},
}
local FarmSpots={
    {n="🍕 Feed",p=CFrame.new(-126,30,-1664),w=6},{n="🎮 Play",p=CFrame.new(-447,30,-1729),w=6},
    {n="🛁 Bath",p=CFrame.new(85,30,-1600),w=6},{n="📚 Learn",p=CFrame.new(-305,30,-1504),w=6},
    {n="💊 Cure",p=CFrame.new(-281,30,-1418),w=6},{n="😴 Sleep",p=CFrame.new(-147,30,-1800),w=6},
}
...
-- Notif
local NH=Instance.new("Frame",SG)NH.Size=UDim2.new(0,250,1,-40)NH.Position=UDim2.new(1,-266,0,20)NH.BackgroundTransparency=1 Instance.new("UIListLayout",NH).Padding=UDim.new(0,4)
local nO=0
...
TW:Create(n,TweenInfo.new(0.25,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Size=UDim2.new(1,0,0,40)}):Play()task.delay(du or 2.5,function()TW:Create(n,TweenInfo.new(0.2),{Size=UDim2.new(0,0,0,40)}):Play()task.wait(0.25)n:Destroy()end)end

-- Panel
local P=Instance.new("Frame",SG)P.Size=UDim2.new(0,460,0,400)P.Position=UDim2.new(0.5,-230,0.5,-200)P.BackgroundColor3=T.bg1 P.BorderSizePixel=0 P.ClipsDescendants=true Instance.new("UICorner",P).CornerRadius=UDim.new(0,14)local PSt=Instance.new("UIStroke",P)PSt.Color=T.br

-- Menu particles
for i=1,6 do local pp=Instance.new("Frame",P)pp.Size=UDim2.new(0,math.random(2,4),0,math.random(2,4))pp.Position=UDim2.new(math.random(10,90)/100,0,math.random(15,95)/100,0)pp.BackgroundColor3=Ac pp.BackgroundTransparency=0.88 pp.BorderSizePixel=0 pp.ZIndex=0 Instance.new("UICorner",pp).CornerRadius=UDim.new(1,0)
task.spawn(function()while not Dead and pp.Parent do TW:Create(pp,TweenInfo.new(math.random(8,14),Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{Position=UDim2.new(math.random(10,90)/100,0,math.random(15,95)/100,0)}):Play()task.wait(math.random(8,14))end end)end
...
local hss=Instance.new("TextLabel",H)hss.Size=UDim2.new(0,130,0,10)hss.Position=UDim2.new(0,42,0,21)hss.BackgroundTransparency=1 hss.Text="ULTIMATE v4.0 · 200+" hss.Font=Enum.Font.Gotham hss.TextSize=7 hss.TextColor3=Ac hss.TextXAlignment=Enum.TextXAlignment.Left

local sessLbl=Instance.new("TextLabel",H)sessLbl.Size=UDim2.new(0,50,0,10)sessLbl.Position=UDim2.new(1,-105,0,17)sessLbl.BackgroundTransparency=1 sessLbl.Text="0:00" sessLbl.Font=Enum.Font.Code sessLbl.TextSize=8 sessLbl.TextColor3=T.t3
...
local mii=false mbb.MouseButton1Click:Connect(function()mii=not mii P.Size=mii and UDim2.new(0,460,0,44)or UDim2.new(0,460,0,400)mbb.Text=mii and "+" or "—" end)

-- Sidebar with scroll
local Si=Instance.new("Frame",P)Si.Size=UDim2.new(0,90,1,-50)Si.Position=UDim2.new(0,4,0,46)Si.BackgroundColor3=T.bg2 Si.BorderSizePixel=0 Instance.new("UICorner",Si).CornerRadius=UDim.new(0,8)
...
local TC=Instance.new("ScrollingFrame",P)TC.Size=UDim2.new(1,-102,1,-50)TC.Position=UDim2.new(0,98,0,46)TC.BackgroundTransparency=1 TC.BorderSizePixel=0 TC.ScrollBarThickness=2 TC.AutomaticCanvasSize=Enum.AutomaticSize.Y TC.CanvasSize=UDim2.new(0,0,0,0)TC.ScrollBarImageColor3=Ac
...
local function mkT(n,o)local b=Instance.new("Frame",SiS)b.Size=UDim2.new(1,0,0,20)b.BackgroundTransparency=1 b.LayoutOrder=o local bg=Instance.new("Frame",b)bg.Size=UDim2.new(1,0,1,0)bg.BackgroundTransparency=1 bg.BorderSizePixel=0 Instance.new("UICorner",bg).CornerRadius=UDim.new(0,5)local lb=Instance.new("TextLabel",b)lb.Size=UDim2.new(1,-4,1,0)lb.Position=UDim2.new(0,4,0,0)lb.BackgroundTransparency=1 lb.Text=n lb.Font=Enum.Font.GothamMedium lb.TextSize=8 lb.TextColor3=T.t3 lb.TextXAlignment=Enum.TextXAlignment.Left local bt=Instance.new("TextButton",b)bt.Size=UDim2.new(1,0,1,0)bt.BackgroundTransparency=1 bt.Text="" bt.AutoButtonColor=false bt.MouseButton1Click:Connect(function()selT(n)end)local f=Instance.new("Frame",TC)f.Size=UDim2.new(1,0,0,0)f.BackgroundTransparency=1 f.AutomaticSize=Enum.AutomaticSize.Y f.Visible=false Instance.new("UIListLayout",f).Padding=UDim.new(0,5)tbs[n]={bg=bg,lb=lb}tfs[n]=f return f end

-- 15 TABS
local tPl=mkT("🏃 Player",1)local tFl=mkT("✈️ Fly",2)local tBd=mkT("🧍 Body",3)local tTp=mkT("🏠 Teleport",4)local tFa=mkT("💰 Farm",5)local tCa=mkT("🐾 Pet Care",6)local tEv=mkT("🎪 Events",7)local tEs=mkT("👁️ ESP",8)local tWo=mkT("🌍 World",9)local tCm=mkT("📷 Camera",10)local tAd=mkT("👤 Admin",11)local tSt=mkT("📊 Stats",12)local tTh=mkT("🎨 Themes",13)local tDc=mkT("💬 Discord",14)local tSe=mkT("⚙️ Settings",15)
...
-- ═══ TAB 1: PLAYER ═══
local p1=mS(tPl,"🏃 MOVEMENT",1)mT(p1,"Walk Speed",1,"Speed",function(v)if not v and LP.Character then local h=LP.Character:FindFirstChildOfClass("Humanoid")if h then h.WalkSpeed=16 end end end)mSl(p1,"Speed Value",2,16,200,50,"",function(v)CFG.SV=v end)mT(p1,"Jump Power",3,"JP",function(v)if not v and LP.Character then local h=LP.Character:FindFirstChildOfClass("Humanoid")if h then h.JumpPower=50 end end end)mSl(p1,"Jump Value",4,50,300,100,"",function(v)CFG.JV=v end)mT(p1,"Infinite Jump",5,"InfJump")mT(p1,"Noclip",6,"Noclip")mT(p1,"Anti-AFK",7,"AFK")mT(p1,"God Mode",8,"GodMode")
local p2=mS(tPl,"📍 UTILITY",2)mB(p2,"💾 Save Position",1,function()if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")then CFG.SP=LP.Character.HumanoidRootPart.CFrame Nf("Pos","Saved!",1.5,Ac)end end)mB(p2,"📍 Teleport Back",2,function()if CFG.SP and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")then LP.Character.HumanoidRootPart.CFrame=CFG.SP end end)mB(p2,"🔄 Reset Character",3,function()if LP.Character then local h=LP.Character:FindFirstChildOfClass("Humanoid")if h then h.Health=0 end end end)

-- ═══ TAB 2: FLY ═══
local f1=mS(tFl,"✈️ FLY MODE",1)mT(f1,"Enable Fly",1,"Fly")mSl(f1,"Fly Speed",2,10,200,60,"",function(v)CFG.FS=v end)
...
-- ═══ TAB 3: BODY ═══
local bd1=mS(tBd,"🧍 BODY MODS",1)
mT(bd1,"Tiny Character",1,"Tiny",function(v)if LP.Character then local h=LP.Character:FindFirstChildOfClass("Humanoid")if h then if v then for _,p in ipairs(LP.Character:GetDescendants())do if p:IsA("BasePart")then p.Size=p.Size*0.5 end end else LP.Character:BreakJoints()end end end end)
mT(bd1,"Giant Character",2,"Giant",function(v)if LP.Character then local h=LP.Character:FindFirstChildOfClass("Humanoid")if h then if v then for _,p in ipairs(LP.Character:GetDescendants())do if p:IsA("BasePart")then p.Size=p.Size*2 end end else LP.Character:BreakJoints()end end end end)
mT(bd1,"Invisible",3,"Invis",function(v)if LP.Character then for _,p in ipairs(LP.Character:GetDescendants())do if p:IsA("BasePart")then p.Transparency=v and 1 or 0 end if p:IsA("Decal")or p:IsA("Texture")then p.Transparency=v and 1 or 0 end end if LP.Character:FindFirstChild("Head")then LP.Character.Head.face.Transparency=v and 1 or 0 end end end)
...
local t2=mS(tTp,"🔍 SEARCH TP",2)
mB(t2,"🏪 Salon",1,function()tpTo("salon")end)mB(t2,"☕ Coffee",2,function()tpTo("coffee")end)mB(t2,"🌉 Bridge",3,function()tpTo("bridge")end)mB(t2,"🌳 Treehouse",4,function()tpTo("tree")end)mB(t2,"🍦 Ice Cream",5,function()tpTo("icecream")end)mB(t2,"🏋️ Gym",6,function()tpTo("gym")end)mB(t2,"🏖️ Beach",7,function()tpTo("beach")end)mB(t2,"🎠 Roundabout",8,function()tpTo("roundabout")end)
local t3=mS(tTp,"👤 TP TO PLAYER",3)
local tpL=Instance.new("ScrollingFrame",t3)tpL.Size=UDim2.new(1,0,0,100)tpL.BackgroundColor3=T.bg3 tpL.BorderSizePixel=0 tpL.ScrollBarThickness=2 tpL.ScrollBarImageColor3=Ac tpL.AutomaticCanvasSize=Enum.AutomaticSize.Y tpL.CanvasSize=UDim2.new(0,0,0,0)tpL.LayoutOrder=1 Instance.new("UICorner",tpL).CornerRadius=UDim.new(0,6)Instance.new("UIListLayout",tpL).Padding=UDim.new(0,2)Instance.new("UIPadding",tpL).PaddingTop=UDim.new(0,3)
local function RefTP()for _,c in ipairs(tpL:GetChildren())do if c:IsA("Frame")then c:Destroy()end end for _,p in ipairs(Players:GetPlayers())do if p==LP then continue end local r=Instance.new("Frame",tpL)r.Size=UDim2.new(1,-6,0,20)r.BackgroundColor3=T.bg2 r.BorderSizePixel=0 Instance.new("UICorner",r).CornerRadius=UDim.new(0,4)local n=Instance.new("TextLabel",r)n.Size=UDim2.new(0.6,0,1,0)n.Position=UDim2.new(0,6,0,0)n.BackgroundTransparency=1 n.Text=p.DisplayName n.Font=Enum.Font.GothamBold n.TextSize=8 n.TextColor3=T.t1 n.TextXAlignment=Enum.TextXAlignment.Left n.TextTruncate=Enum.TextTruncate.AtEnd local b=Instance.new("TextButton",r)b.Size=UDim2.new(0,30,0,14)b.Position=UDim2.new(1,-34,0.5,-7)b.BackgroundColor3=Ac b.BorderSizePixel=0 b.Text="TP" b.Font=Enum.Font.GothamBold b.TextSize=7 b.TextColor3=T.t1 b.AutoButtonColor=false Instance.new("UICorner",b).CornerRadius=UDim.new(0,3)b.MouseButton1Click:Connect(function()if LP.Character and p.Character and LP.Character:FindFirstChild("HumanoidRootPart")and p.Character:FindFirstChild("HumanoidRootPart")then LP.Character.HumanoidRootPart.CFrame=p.Character.HumanoidRootPart.CFrame+Vector3.new(0,0,3)Nf("TP","→ "..p.Name,1.5,Ac)end end)end end
mB(t3,"🔄 Refresh",2,RefTP)RefTP()

-- ═══ TAB 5: FARM ═══
local fa1=mS(tFa,"🔄 AUTO FARM CYCLE",1)
local farmStatus=Instance.new("TextLabel",fa1)farmStatus.Size=UDim2.new(1,0,0,14)farmStatus.BackgroundTransparency=1 farmStatus.Text="⏻ Farm OFF" farmStatus.Font=Enum.Font.GothamBold farmStatus.TextSize=11 farmStatus.TextColor3=Color3.fromRGB(255,50,50)farmStatus.TextXAlignment=Enum.TextXAlignment.Left farmStatus.LayoutOrder=1
local farmBtn=Instance.new("TextButton",fa1)farmBtn.Size=UDim2.new(1,0,0,26)farmBtn.BackgroundColor3=Color3.fromRGB(0,200,100)farmBtn.BorderSizePixel=0 farmBtn.Text="▶ START AUTO FARM" farmBtn.Font=Enum.Font.GothamBold farmBtn.TextSize=10 farmBtn.TextColor3=Color3.new(1,1,1)farmBtn.AutoButtonColor=false farmBtn.LayoutOrder=2 Instance.new("UICorner",farmBtn).CornerRadius=UDim.new(0,6)
mSl(fa1,"Wait per spot",3,3,20,8,"s",function(v)CFG.CareSpd=v end)
local farmCycleLbl=Instance.new("TextLabel",fa1)farmCycleLbl.Size=UDim2.new(1,0,0,12)farmCycleLbl.BackgroundTransparency=1 farmCycleLbl.Text="🔄 Cycles: 0 · 💰 Est: ~0 Bucks" farmCycleLbl.Font=Enum.Font.GothamBold farmCycleLbl.TextSize=9 farmCycleLbl.TextColor3=Color3.fromRGB(255,220,0)farmCycleLbl.TextXAlignment=Enum.TextXAlignment.Left farmCycleLbl.LayoutOrder=4
local farmSpotLbl=Instance.new("TextLabel",fa1)farmSpotLbl.Size=UDim2.new(1,0,0,11)farmSpotLbl.BackgroundTransparency=1 farmSpotLbl.Text="📍 Waiting..." farmSpotLbl.Font=Enum.Font.Gotham farmSpotLbl.TextSize=8 farmSpotLbl.TextColor3=T.t3 farmSpotLbl.TextXAlignment=Enum.TextXAlignment.Left farmSpotLbl.LayoutOrder=5

local function togFarm()AutoFarm=not AutoFarm if AutoFarm then farmBtn.Text="⏹ STOP" farmBtn.BackgroundColor3=Color3.fromRGB(255,50,50)farmStatus.Text="⏻ FARMING..." farmStatus.TextColor3=Color3.fromRGB(0,255,130)FarmIdx=1 else farmBtn.Text="▶ START AUTO FARM" farmBtn.BackgroundColor3=Color3.fromRGB(0,200,100)farmStatus.Text="⏻ Farm OFF" farmStatus.TextColor3=Color3.fromRGB(255,50,50)farmSpotLbl.Text="📍 Stopped" end end
farmBtn.MouseButton1Click:Connect(togFarm)

local fa2=mS(tFa,"💰 AUTO COLLECT",2)mT(fa2,"Collect Items",1,"AutoCollect")mSl(fa2,"Range",2,10,200,50,"m",function(v)CFG.CR=v end)mSl(fa2,"Speed",3,1,10,3,"s",function(v)CFG.CS=v end)
local colLbl=Instance.new("TextLabel",fa2)colLbl.Size=UDim2.new(1,0,0,12)colLbl.BackgroundTransparency=1 colLbl.Text="💰 Collected: 0" colLbl.Font=Enum.Font.GothamBold colLbl.TextSize=9 colLbl.TextColor3=Color3.fromRGB(255,220,0)colLbl.LayoutOrder=4

local fa3=mS(tFa,"🎯 MANUAL FARM",3)
for i,spot in ipairs(FarmSpots)do mB(fa3,spot.n,i,function()if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")then LP.Character.HumanoidRootPart.CFrame=spot.p Nf("Farm","→ "..spot.n,1.5,Ac)end end)end

-- ═══ TAB 6: PET CARE ═══
local ca1=mS(tCa,"🐾 AUTO PET CARE",1)mT(ca1,"Auto Care Cycle",1,"AutoCare")mSl(ca1,"Care Speed",2,4,20,8,"s",function(v)CFG.CareSpd=v end)
local careLbl=Instance.new("TextLabel",ca1)careLbl.Size=UDim2.new(1,0,0,12)careLbl.BackgroundTransparency=1 careLbl.Text="🐾 Care: 0" careLbl.Font=Enum.Font.GothamBold careLbl.TextSize=9 careLbl.TextColor3=Color3.fromRGB(255,180,0)careLbl.LayoutOrder=3
local ca2=mS(tCa,"🍖 QUICK CARE",2)
mB(ca2,"🍕 Feed (Pizza)",1,function()if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")then LP.Character.HumanoidRootPart.CFrame=CFrame.new(-126,30,-1664)Nf("Care","→ Feed",1.5,Ac)end end)
mB(ca2,"😴 Sleep (Camp)",2,function()if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")then LP.Character.HumanoidRootPart.CFrame=CFrame.new(-147,30,-1800)Nf("Care","→ Sleep",1.5,Ac)end end)
mB(ca2,"🛁 Bath (Pool)",3,function()if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")then LP.Character.HumanoidRootPart.CFrame=CFrame.new(85,30,-1600)Nf("Care","→ Bath",1.5,Ac)end end)
mB(ca2,"🎮 Play (Park)",4,function()if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")then LP.Character.HumanoidRootPart.CFrame=CFrame.new(-447,30,-1729)Nf("Care","→ Play",1.5,Ac)end end)
mB(ca2,"📚 Learn (School)",5,function()if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")then LP.Character.HumanoidRootPart.CFrame=CFrame.new(-305,30,-1504)Nf("Care","→ Learn",1.5,Ac)end end)
mB(ca2,"💊 Cure (Hospital)",6,function()if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")then LP.Character.HumanoidRootPart.CFrame=CFrame.new(-281,30,-1418)Nf("Care","→ Cure",1.5,Ac)end end)

-- ═══ TAB 7: EVENTS ═══
local ev1=mS(tEv,"🚢 CURRENT EVENT",1)mB(ev1,"🚢 Boat/Ship",1,function()tpTo("boat")end)mB(ev1,"🦈 Shark",2,function()tpTo("shark")end)mB(ev1,"🤿 Dive",3,function()tpTo("dive")end)mB(ev1,"🗑️ Underwater",4,function()tpTo("underwater")end)mB(ev1,"🧭 Compass",5,function()tpTo("compass")end)mB(ev1,"🩺 Nurse",6,function()tpTo("nurse")end)
local ev2=mS(tEv,"🏆 COLLECTIBLES",2)mB(ev2,"🌟 Stumps",1,function()tpTo("stump")end)mB(ev2,"🏳️‍🌈 Pride",2,function()tpTo("pride")end)mB(ev2,"🌸 Flowers",3,function()tpTo("flower")end)mB(ev2,"🗺️ Journey",4,function()tpTo("journey")end)mB(ev2,"📦 Chest",5,function()tpTo("treasure")end)
local ev3=mS(tEv,"🔍 SCANNER",3)mB(ev3,"🔎 Scan Items",1,function()local items=findCollectibles()Nf("Scan","Found "..#items.." items!",3,Color3.fromRGB(255,220,0))end)mB(ev3,"🔎 Scan Prompts",2,function()local c=0 for _,v in ipairs(workspace:GetDescendants())do if v:IsA("ProximityPrompt")then c=c+1 end end Nf("Scan",c.." prompts",3,Ac)end)

-- ═══ TAB 8: ESP ═══
local es1=mS(tEs,"👁️ PLAYER ESP",1)mT(es1,"Names",1,"ESPNm")mT(es1,"Box",2,"ESPBx")mT(es1,"Distance",3,"ESPDst")mT(es1,"Tracers",4,"ESPTr")mT(es1,"Health Bar",5,"ESPHp")mT(es1,"Rainbow ESP",6,"RbESP")
...
mT(w1,"Night Mode",5,"Night",function(v)if v then Light.ClockTime=0 Light.Brightness=0.5 else Light.ClockTime=OG.CT Light.Brightness=OG.B end end)
mT(w1,"Sunset Mode",6,"Sunset",function(v)if v then Light.ClockTime=18 Light.Brightness=1 else Light.ClockTime=OG.CT Light.Brightness=OG.B end end)
mT(w1,"No Shadows",7,"NoShadow",function(v)Light.GlobalShadows=not v end)

-- ═══ TAB 10: CAMERA ═══
local cm1=mS(tCm,"📷 CAMERA",1)mT(cm1,"FOV Changer",1,"FvC",function(v)if not v then Cam.FieldOfView=OG.CV end end)mSl(cm1,"FOV",2,30,140,70,"°",function(v)CFG.CF=v end)
...
local function RefAd()for _,c in ipairs(adL:GetChildren())do if c:IsA("Frame")then c:Destroy()end end for _,p in ipairs(Players:GetPlayers())do if p==LP then continue end local r=Instance.new("Frame",adL)r.Size=UDim2.new(1,-6,0,24)r.BackgroundColor3=T.bg2 r.BorderSizePixel=0 Instance.new("UICorner",r).CornerRadius=UDim.new(0,4)local n=Instance.new("TextLabel",r)n.Size=UDim2.new(0.4,0,1,0)n.Position=UDim2.new(0,6,0,0)n.BackgroundTransparency=1 n.Text=p.DisplayName n.Font=Enum.Font.GothamBold n.TextSize=8 n.TextColor3=T.t1 n.TextXAlignment=Enum.TextXAlignment.Left n.TextTruncate=Enum.TextTruncate.AtEnd
...
-- ═══ TAB 12: STATS ═══
local st1=mS(tSt,"📊 SESSION",1)
local statSession=Instance.new("TextLabel",st1)statSession.Size=UDim2.new(1,0,0,60)statSession.BackgroundTransparency=1 statSession.Text="⏱ Session: 0:00\n🔄 Farm Cycles: 0\n💰 Est. Bucks: ~0\n🐾 Care Actions: 0\n📦 Items Collected: 0" statSession.Font=Enum.Font.Code statSession.TextSize=9 statSession.TextColor3=T.t2 statSession.TextXAlignment=Enum.TextXAlignment.Left statSession.TextYAlignment=Enum.TextYAlignment.Top statSession.LayoutOrder=1

-- ═══ TAB 13: THEMES ═══
local th1=mS(tTh,"🎨 ACCENT COLOR",1)
local acR=Instance.new("Frame",th1)acR.Size=UDim2.new(1,0,0,22)acR.BackgroundTransparency=1 acR.LayoutOrder=1 Instance.new("UIListLayout",acR).FillDirection=Enum.FillDirection.Horizontal Instance.new("UIListLayout",acR).Padding=UDim.new(0,5)
for _,c in ipairs({{Color3.fromRGB(0,200,100)},{Color3.fromRGB(40,140,255)},{Color3.fromRGB(255,60,80)},{Color3.fromRGB(255,200,0)},{Color3.fromRGB(255,80,180)},{Color3.fromRGB(0,220,255)},{Color3.fromRGB(150,0,255)},{Color3.fromRGB(255,140,0)}})do local sw=Instance.new("TextButton",acR)sw.Size=UDim2.new(0,26,0,20)sw.BackgroundColor3=c[1]sw.BorderSizePixel=0 sw.Text="" sw.AutoButtonColor=false Instance.new("UICorner",sw).CornerRadius=UDim.new(0,5)sw.MouseButton1Click:Connect(function()Ac=c[1]TC.ScrollBarImageColor3=c[1]lo.BackgroundColor3=c[1]Nf("Theme","Changed!",1.5,c[1])end)end
local th2=mS(tTh,"🎭 MENU THEME",2)
local thR=Instance.new("Frame",th2)thR.Size=UDim2.new(1,0,0,22)thR.BackgroundTransparency=1 thR.LayoutOrder=1 Instance.new("UIListLayout",thR).FillDirection=Enum.FillDirection.Horizontal Instance.new("UIListLayout",thR).Padding=UDim.new(0,4)
for _,mc in ipairs({{n="Dark",b1=Color3.fromRGB(8,12,10),b2=Color3.fromRGB(12,20,16),br=Color3.fromRGB(30,55,42)},{n="Night",b1=Color3.fromRGB(5,10,20),b2=Color3.fromRGB(10,18,35),br=Color3.fromRGB(30,45,75)},{n="Void",b1=Color3.fromRGB(3,3,8),b2=Color3.fromRGB(7,5,14),br=Color3.fromRGB(25,20,45)},{n="Blood",b1=Color3.fromRGB(14,4,4),b2=Color3.fromRGB(24,8,8),br=Color3.fromRGB(60,22,22)},{n="Ocean",b1=Color3.fromRGB(4,8,18),b2=Color3.fromRGB(8,14,30),br=Color3.fromRGB(25,40,70)}})do local sw=Instance.new("TextButton",thR)sw.Size=UDim2.new(0,44,0,20)sw.BackgroundColor3=mc.b2 sw.BorderSizePixel=0 sw.Text=mc.n sw.Font=Enum.Font.GothamBold sw.TextSize=7 sw.TextColor3=T.t2 sw.AutoButtonColor=false Instance.new("UICorner",sw).CornerRadius=UDim.new(0,4)sw.MouseButton1Click:Connect(function()T.bg1=mc.b1 T.bg2=mc.b2 T.br=mc.br local ti=TweenInfo.new(0.3)TW:Create(P,ti,{BackgroundColor3=mc.b1}):Play()TW:Create(H,ti,{BackgroundColor3=mc.b2}):Play()TW:Create(HX,ti,{BackgroundColor3=mc.b2}):Play()TW:Create(Si,ti,{BackgroundColor3=mc.b2}):Play()PSt.Color=mc.br for _,tf in pairs(tfs)do for _,ch in ipairs(tf:GetChildren())do if ch:IsA("Frame")and ch.BackgroundTransparency<0.5 then TW:Create(ch,ti,{BackgroundColor3=mc.b2}):Play()local stk=ch:FindFirstChildOfClass("UIStroke")if stk then TW:Create(stk,ti,{Color=mc.br}):Play()end end end end Nf("Theme",mc.n,1.5,Ac)end)end

-- ═══ TAB 14: DISCORD ═══
local dc1=mS(tDc,"💬 DISCORD",1)
mB(dc1,"📋 Copy Discord Link",1,function()if setclipboard then setclipboard("https://discord.gg/qTGtM4Uyc")Nf("Discord","Link copied!",2,Color3.fromRGB(88,101,242))else Nf("Error","Not supported",2,Color3.fromRGB(255,50,50))end end)
local dcInfo=Instance.new("TextLabel",dc1)dcInfo.Size=UDim2.new(1,0,0,30)dcInfo.BackgroundTransparency=1 dcInfo.Text="Join our Discord for:\n• Updates · Support · New scripts\ndiscord.gg/qTGtM4Uyc" dcInfo.Font=Enum.Font.Gotham dcInfo.TextSize=8 dcInfo.TextColor3=Color3.fromRGB(88,101,242)dcInfo.TextXAlignment=Enum.TextXAlignment.Left dcInfo.TextYAlignment=Enum.TextYAlignment.Top dcInfo.LayoutOrder=2

-- ═══ TAB 15: SETTINGS ═══
local se1=mS(tSe,"👤 PROFILE",1)
local pRow=Instance.new("Frame",se1)pRow.Size=UDim2.new(1,0,0,50)pRow.BackgroundTransparency=1 pRow.LayoutOrder=1
...
Instance.new("TextLabel",pRow).Size=UDim2.new(1,-50,0,10) local pu=pRow:GetChildren()[#pRow:GetChildren()]pu.Position=UDim2.new(0,50,0,19)pu.BackgroundTransparency=1 pu.Text="@"..LP.Name pu.Font=Enum.Font.Gotham pu.TextSize=8 pu.TextColor3=T.t3 pu.TextXAlignment=Enum.TextXAlignment.Left
Instance.new("TextLabel",pRow).Size=UDim2.new(1,-50,0,10) local pst=pRow:GetChildren()[#pRow:GetChildren()]pst.Position=UDim2.new(0,50,0,32)pst.BackgroundTransparency=1 pst.Text="● PREMIUM ACTIVE" pst.Font=Enum.Font.GothamBold pst.TextSize=8 pst.TextColor3=Color3.fromRGB(0,255,130)pst.TextXAlignment=Enum.TextXAlignment.Left

local se2=mS(tSe,"ℹ️ INFO",2)
local il=Instance.new("TextLabel",se2)il.Size=UDim2.new(1,0,0,60)il.BackgroundTransparency=1 il.Text="[X] toggle farm · [LCtrl] hide menu\n[F] save pos · [G] tp back · [Del] destroy\n\nHIROSHI 738 · Adopt Me Ultimate v4.0\n200+ Features · 15 Tabs\nBy Hiroshi738 · discord.gg/qTGtM4Uyc" il.Font=Enum.Font.Gotham il.TextSize=8 il.TextColor3=T.t2 il.TextXAlignment=Enum.TextXAlignment.Left il.TextYAlignment=Enum.TextYAlignment.Top il.LayoutOrder=1

mB(se2,"🔄 Reset All Settings",2,function()for k,v in pairs(CFG)do if type(v)=="boolean" then CFG[k]=false end end CFG.AFK=true Nf("Reset","Done!",2,Color3.fromRGB(255,220,0))end)
mB(se2,"📋 Copy Loader",3,function()if setclipboard then setclipboard('loadstring(game:HttpGet("https://raw.githubusercontent.com/yann56100mattera-star/AdoptMe/main/main.lua"))()')Nf("Copied","Loader copied!",2,Ac)end end)

selT("🏃 Player")

-- ═══ SYSTEMS ═══
table.insert(CN,UIS.InputBegan:Connect(function(i,g)if g then return end
    if i.KeyCode==Enum.KeyCode.X then togFarm()
    elseif i.KeyCode==Enum.KeyCode.LeftControl then P.Visible=not P.Visible
    elseif i.KeyCode==Enum.KeyCode.Delete then Dead=true AutoFarm=false for _,c in ipairs(CN)do pcall(function()c:Disconnect()end)end
        if Spec then Cam.CameraSubject=LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")end
        if LP.Character then local hr=LP.Character:FindFirstChild("HumanoidRootPart")if hr then local bv=hr:FindFirstChild("H738F")if bv then bv:Destroy()end local bg=hr:FindFirstChild("H738G")if bg then bg:Destroy()end end local hm=LP.Character:FindFirstChildOfClass("Humanoid")if hm then pcall(function()hm.WalkSpeed=16 hm.JumpPower=50 hm.PlatformStand=false end)end end
        pcall(function()Light.Brightness=OG.B Light.FogEnd=OG.FE Light.FogStart=OG.FS2 Light.GlobalShadows=OG.GS Light.ClockTime=OG.CT Cam.FieldOfView=OG.CV end)
        for p,o in pairs(EO)do pcall(function()o.nm:Remove()o.dt:Remove()o.bx:Remove()o.tr:Remove()end)end SG:Destroy()
...
-- Auto Care
local careSpots2={CFrame.new(-126,30,-1664),CFrame.new(-447,30,-1729),CFrame.new(85,30,-1600),CFrame.new(-305,30,-1504),CFrame.new(-281,30,-1418),CFrame.new(-147,30,-1800)}
local careIdx=1 local lastCare=0
table.insert(CN,RS.Heartbeat:Connect(function()if Dead or not CFG.AutoCare then return end local now=tick()if now-lastCare<CFG.CareSpd then return end lastCare=now
    if LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")then LP.Character.HumanoidRootPart.CFrame=careSpots2[careIdx]careIdx=careIdx%#careSpots2+1 careCount=careCount+1 careLbl.Text="🐾 Care: "..careCount end end))

-- Auto Farm Loop
task.spawn(function()while not Dead do
    if AutoFarm and LP.Character and LP.Character:FindFirstChild("HumanoidRootPart") then
        local spot=FarmSpots[FarmIdx]farmSpotLbl.Text="📍 → "..spot.n
        LP.Character.HumanoidRootPart.CFrame=spot.p
        for sec=CFG.CareSpd,1,-1 do if not AutoFarm or Dead then break end farmSpotLbl.Text="📍 "..spot.n.." ("..sec.."s)" task.wait(1)end
        if not AutoFarm or Dead then continue end
        FarmIdx=FarmIdx+1
        if FarmIdx>#FarmSpots then FarmIdx=1 farmCycles=farmCycles+1 end
        farmCycleLbl.Text="🔄 Cycles: "..farmCycles.." · 💰 Est: ~"..(farmCycles*30).." Bucks"
...
table.insert(CN,RS.RenderStepped:Connect(function(dt)if Dead then return end
    ...
    if hm then
        if CFG.Speed then hm.WalkSpeed=CFG.SV end
        if CFG.JP then hm.JumpPower=CFG.JV end
        if CFG.GodMode then hm.Health=hm.MaxHealth end
    end
    ...
    if CFG.RbESP then rbH=(rbH+dt*0.3)%1 end

    local el=math.floor(tick()-Session)sessLbl.Text=math.floor(el/60)..":"..string.format("%02d",el%60)
    statSession.Text="⏱ Session: "..math.floor(el/60)..":"..string.format("%02d",el%60).."\n🔄 Farm Cycles: "..farmCycles.."\n💰 Est. Bucks: ~"..(farmCycles*30).."\n🐾 Care Actions: "..careCount.."\n📦 Items Collected: "..collectCount

    local vp=Cam.ViewportSize local ctr=vp/2
    local anyE=CFG.ESPNm or CFG.ESPBx or CFG.ESPDst or CFG.ESPTr
    for pi,p in ipairs(Players:GetPlayers())do if p==LP then continue end local o=gE(p)
        ...
        local col=CFG.RbESP and Color3.fromHSV((rbH+pi*0.1)%1,0.8,1)or Ac
        if CFG.ESPNm then o.nm.Text=p.DisplayName o.nm.Position=Vector2.new(sp.X,sp.Y-16)o.nm.Color=col o.nm.Visible=true else o.nm.Visible=false end
        if CFG.ESPDst then o.dt.Text="["..math.floor(dist).."m]" o.dt.Position=Vector2.new(fp.X,fp.Y+3)o.dt.Color=col o.dt.Visible=true else o.dt.Visible=false end
        if CFG.ESPBx then o.bx.Size=Vector2.new(bW,bH)o.bx.Position=Vector2.new(bX,bY)o.bx.Color=col o.bx.Visible=true else o.bx.Visible=false end
        if CFG.ESPTr then o.tr.From=Vector2.new(ctr.X,vp.Y)o.tr.To=Vector2.new(fp.X,fp.Y)o.tr.Color=col o.tr.Visible=true else o.tr.Visible=false end
    end
end))

Nf("🐾 HIROSHI 738","Ultimate Premium v4.0!",3,Ac)
Nf("💰 Features","15 tabs · 200+ features",3,Color3.fromRGB(255,220,0))
Nf("✈️ Ready","Press X for auto-farm",3,Color3.fromRGB(0,180,255))
